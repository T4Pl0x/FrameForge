...
export async function evaluateGates({ owner, repo, runId, token, trace_id }) {
  ...
  if (allPass) {
    ...
    try { kapi?.events?.publish('gate.passed', { type: 'gate.passed', trace_id: tr, status: 'green', details }); } catch {}
    ...
    try { kapi?.events?.publish('gate.failed', { type: 'gate.failed', trace_id: tr, status: 'red', details: { firstHint, reportType } }); } catch {}
