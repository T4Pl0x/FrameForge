export type AppId = "publish"|"agents"|"tools"|"settings"|"processes"|"logs"|"approvals"|"prompt-lab"|"builder";
...
class WindowManager {
  ...
  snap(id: WinId, pos: "left"|"right"|"max"){
    const w = this.windows.find(w => w.id === id);
    if (!w) return;
    const vw = window.innerWidth; const vh = window.innerHeight - 36; // account taskbar
