...
function Content({ w }: { w: WinSpec }){
  switch (w.app){
    case 'publish':   return <div className="muted">Publish (window)</div>;
...
export function Desktop({ noDock }: { noDock?: boolean }){
  const wins = useWindows();
  const active = useActive();
  return (
    <div className={"ff-desktop" + (noDock ? " no-dock" : "") }>
      {wins.filter(w => !w.minimized).map(w => (
        <Window key={w.id} w={w} active={active === w.id} />
      ))}
    </div>
  );
}

function Window({ w, active }: { w: WinSpec, active?: boolean }){
